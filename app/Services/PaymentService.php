...
class PaymentService
{
    ...
    public function __construct($method, Payment $payment = null)
    {
        ...
        if (isset($payment)) {
            ...
            $this->_config['notify_domain'] = $payment->getAttribute(Payment::FIELD_NOTIFY_DOMAIN);
        }
    ...
    {
        $notifyUrl = url("/api/v1/guest/payment/notify/{$this->_method}/{$this->_config['uuid']}");
        if ($this->_config['notify_domain']) {
            $parseUrl = parse_url($notifyUrl);
            $notifyUrl = $this->_config['notify_domain'] . $parseUrl['path'];
        }
